...
def process_alphapose(alphapose_raw, label = False, get_num = False):
    """
    Recieves the Alphapose result and transforms into a format easier to work with.
    alphapose_raw: Contains raw Alphapose result. Assumed to be in COCO format.
...
def main():
    args = get_args()
    json_list = get_json_list(args.inpath)
    for json_file in json_list:
        data_name = Path(json_file).stem
        alphapose_raw = get_json_file(json_file, args.inpath)
        alphapose_results = process_alphapose(alphapose_raw)
    
        person_to_body_id = identify_people(alphapose_results)
        keypoints_matched = match_keypoints(person_to_body_id, alphapose_results)
        save_analysis(keypoints_matched, data_name = f"{data_name}_person", outpath = args.outpath)
